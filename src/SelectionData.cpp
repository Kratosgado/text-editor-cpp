...
void SelectionData::updateLastSelection(int extremoLine, int extremoChar) {
   if (this->lastSelectionIndex < 0) return;
   int anclaLine = this->getLastAnclaLine();
   int anclaChar = this->getLastAnclaChar();

   bool nuevaActive = (anclaLine != extremoLine) || (anclaChar != extremoChar);
   this->selections[this->lastSelectionIndex].activa = nuevaActive;
